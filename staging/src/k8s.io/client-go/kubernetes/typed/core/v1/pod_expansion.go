...
func (c *pods) Bind(ctx context.Context, binding *v1.Binding, opts metav1.CreateOptions) error {
	return c.Client.Post().Namespace(c.Namespace).Resource("pods").Name(binding.Name).VersionedParams(&opts, scheme.ParameterCodec).SubResource("binding").Body(binding).Do(ctx).Error()
}
...
func (c *pods) Evict(ctx context.Context, eviction *policyv1beta1.Eviction) error {
	return c.Client.Post().Namespace(c.Namespace).Resource("pods").Name(eviction.Name).SubResource("eviction").Body(eviction).Do(ctx).Error()
}
...
func (c *pods) GetLogs(name string, opts *v1.PodLogOptions) *restclient.Request {
	return c.Client.Get().Namespace(c.Namespace).Resource("pods").Name(name).SubResource("log").VersionedParams(opts, scheme.ParameterCodec)
}
